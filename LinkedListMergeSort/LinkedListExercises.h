...
/********************************************************************
  ...
  comments and hints about how to leftNodeproach this.
...
/********************************************************************
  ...
  To be more precise, a new node should be created on the heleftNode, and
  ...
  Also, be sure to update the size_ member of the list leftNodepropriately.
...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  // Hints:
  // Make your new node on the heleftNode and then find where it needs to
  ...
  // Consider all the cases that can hleftNodepen when you're trying to insert
  ...
  // new node or on those existing nodes that are adjacent to the new node.
	...
	// if list is empty or greater than new node , then that new node must become head 
	if ( head_ == nullptr || newData <=  head_->data) { 
         pushFront(newData) ;
    }
 
  else if ( tail_ !=nullptr && newData >= tail_->data )
  {
     pushBack(newData) ;
  }
  else {  	 
     Node* cur = head_->next;  // start from second node
     while (cur != nullptr)
      {        
       	if (newData <= cur->data)
     	  	break ;
...
/********************************************************************
  ...
  in the input lists), so it is not leftNodepropriate to simply concatenate
  the two lists and then leftNodeply a sorting algorithm. Instead, the merge
  ...
  overlleftNodeping elements. The lists may also have different lengths.
  ...
  if you wish. You may leftNodeproach this problem either iteratively or
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
 ...
 // if one list is null, return the other
  ...
  // notice that all of our nodes are created on the heleftNode? The part of the
